...
public class IronRod {
  ...
  // 해결방법
  // 스택에 문자가 '('만나면  push 해준다.
  // 이때 스택에 저장된 '('의 수는 현재 레이저가 뚫을 수 있는 막대의 수이다.
  // 만약 '(' 다음 바로 ')'가 나온다면 그것은 막대가 아닌 레이저
  // 이런 경우는 '('하나를 pop 하고 남은 막대를 레이저로 자르기 때문에 스택size만큼 조각이 생기므로 그만큼 더해준다.
  // ')'가 연속으로 나온 경우는 젤 위에 있는 막대가 끝났다는 의미이므로 스택을 하나 제거하고 조각을 +1해준다.
  public int solution(String str) {
    ...
    for (int i = 0; i < str.length(); i++) {
      ...
      else { // ')'인 경우는 바로 앞이 '('인지 ')'에 따라 다르게
        stack.pop(); // 스택을 하나 제거하고 더해주므로 미리 pop
        // '('라면 레이저이므로 스택사이즈만큼 +
        if (str.charAt(i - 1) == '(') answer += stack.size();
        else answer++; // ')'라면 막대가 하나 끝났으므로 +1
      }
    }
    return answer;
  }
